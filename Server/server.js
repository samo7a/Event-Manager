...
app.post("/api/signup", async (req, res) => {
  ...
  let regstrType = req.body.registerType;
...
app.post("/api/getAllRsosStudent", async (req, res) => {
  const { s_id } = req.body;
  let u_id;
  let rso = {};
  let obj = {};
...
app.post("/api/getAllMyRsosStudent", async (req, res) => {
  ...
  conn.query(sql, async (error, result) => {
    ...
    u_id = result[0].u_id;
    sql = `select rso_id, s_id, rso_name, status, rso_description from Rso where s_id=${s_id};`;
    conn.query(sql, async (error1, result1) => {
      if (error1) {
        res.status(401).json({ msg: error1.sqlMessage });
      }
      ...
      sql = `select s_id, s_firstName, s_lastName from Students where s_id=${s_id};`;
      ...
      conn.query(sql, async (error2, result2) => {
        ...
        for (var j = 0; j < result1.length; j++) {
          admins.push({
            s_id: s_id,
            s_firstName: result2[0].s_firstName,
            s_lastName: result2[0].s_lastName,
          });
        }
